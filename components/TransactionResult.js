...
import {Grid, Message, Table, Pagination, Header} from 'semantic-ui-react'
  ...
  <Grid.Row>
    <Grid.Column>
      <Message style={{
        ...
        {
          (props.blockNo || props.timeStamp || props.value) && (<Table celled>
            <Table.Header>
              <Table.Row>
            ...
            <Table.Body>
              <Table.Row>
                ...
                {props.timeStamp && <Table.Cell>{(new Date(Number(props.timeStamp) * 1000)).toString()}</Table.Cell>}
  ...
  render() {
    return (<Grid.Row>
      <Grid.Column>
        ...
        {
          this.props.transactions.filter((transaction, i) => {
